      ...
      <div class="row w-100 mx-0 pt-2">
        <div class="col-lg-4 mx-auto">
          <div class="auth-form-light text-left py-5 px-4 px-sm-5">
            ...
            <h6 class="fw-light">Sign in to continue as User.</h6>
            <form class='pt-3' method="post" action="{{ $url }}">
              {{ csrf_field() }}

              <div class="form-group">
                <label for="customer_id" class="text-primary">Customer Id :</label>
                <input type="text" class="form-control form-control-lg" id="customer_id" name="customer_id"
                  placeholder="Customer Id" value="{{ old('customer_id', $customer_id) }}">
                @if ($errors->has('customer_id'))
                  <span class="text-danger text-small">{{ $errors->first('customer_id') }}</span>
                @endif
              </div>
